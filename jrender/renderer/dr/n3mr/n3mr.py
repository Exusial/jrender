...
class RasterizeVetexFunction(RasterizeFunction):
    def __init__(self, image_size, near, far, eps, background_color, return_rgb=False, return_alpha=False, return_depth=False):
        super(RasterizeVetexFunction, self).__init__(image_size, near, far, eps, background_color, return_rgb, return_alpha, return_depth)

    def grad(self, grad_rgb_map, grad_alpha_map, grad_depth_map):
        faces, vertex_colors, face_index_map, weight_map, depth_map, rgb_map, alpha_map, face_inv_map, sampling_index_map, sampling_weight_map, face_index = self.save_vars

        grad_faces = jt.zeros(faces.shape).float32()
        if self.return_rgb:
            grad_vertex_color = jt.zeros(vertex_colors.shape).float32()
        else:
            grad_vertex_color = jt.zeros(1).float32()
        ...
        # backward pass
        grad_faces = self.backward_pixel_map(faces, face_index_map, rgb_map, alpha_map, grad_rgb_map, grad_alpha_map, grad_faces)
        # claculate grad in python: color = w * vertex_color
        B, H, W, _ = grad_rgb_map.shape
        grad_pixel_vertices = (weight_map[...,None] * grad_rgb_map[...,None,:]).reshape(B, H*W,3,-1)
        # B, P, 3, 3
        grad_vertex_color = grad_pixel_vertices.reindex_reduce("add", vertex_colors.shape, ['@e0(i0, @e1(i0,i1), i2)', 'i3'], extras=[face_index, face_index_map.reshape(B, -1)])
        # grad_textures = self.backward_textures(face_index_map, sampling_weight_map, sampling_index_map, grad_rgb_map, grad_textures)
        grad_faces = self.backward_depth_map(faces, depth_map, face_index_map, face_inv_map, weight_map, grad_depth_map, grad_faces)[0]
        ...
        if grad_faces is not None:
            grad_faces.sync()
        if grad_vertex_color is not None:
            grad_vertex_color.sync()

        return grad_faces, grad_vertex_color

    def execute(self, faces, vertex_colors, face_index):
        self.batch_size, self.num_faces = faces.shape[:2]
        ...
        # faces, textures, face_index_map, weight_map, depth_map -> rgb_map, sampling_index_map, sampling_weight_map
        rgb_map, sampling_index_map, sampling_weight_map = self.forward_vertex_color(face_index, vertex_colors, face_index_map, weight_map, rgb_map, sampling_index_map, sampling_weight_map)
        rgb_map = self.forward_background(face_index_map, rgb_map)

        alpha_map = self.forward_alpha_map(alpha_map, face_index_map)

        self.save_vars = faces, vertex_colors, face_index_map, weight_map, depth_map, rgb_map, alpha_map, face_inv_map, sampling_index_map, sampling_weight_map, face_index
    ...
    def forward_vertex_color(self, faces_index, vertex_color, face_index_map, weight_map, rgb_map, sampling_index_map, sampling_weight_map):
        if not self.return_rgb:
            return rgb_map, sampling_index_map, sampling_weight_map
        else:
            B, f, _ = faces_index.shape
            p = face_index_map.shape[1]
            print(faces_index.shape, face_index_map.shape)
            pixel_color = vertex_color.reindex([B, p, 3, 3], ['@e0(i0, @e1(i0, i1), i2)', 'i3'], extras=[faces_index, face_index_map.reshape(B, -1)])
            rgb_map = (weight_map[...,None] * pixel_color).sum(-1) 
            return rgb_map, sampling_index_map, sampling_weight_map
    
def rasterize_vertex_color(
        faces,
        vertex_colors,
        face_index,
        image_size=DEFAULT_IMAGE_SIZE,
    ...
    if vertex_colors is None:
        ...
        inputs = [faces, vertex_colors, face_index]
    if anti_aliasing:
        # 2x super-sampling
        rgb, alpha, depth = RasterizeVetexFunction(image_size * 2, near, far, eps, background_color, return_rgb, return_alpha, return_depth)(*inputs)
    else:
        rgb, alpha, depth = RasterizeVetexFunction(image_size, near, far, eps, background_color, return_rgb, return_alpha, return_depth)(*inputs)
